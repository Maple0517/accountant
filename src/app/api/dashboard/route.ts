  ...
  try {
    ...
    const baseRequests = [
        ...
        supabase
          .from('transactions')
          .select(
            'id, merchant_name, description, amount, iso_currency_code, date, effective_date, source, pending, tags, treatment, refund_source, transaction_kind, budget_behavior, transfer_match_status, deleted_at, is_hidden_from_reports, split_role, accounts!transactions_account_id_fkey ( name, mask ), categories!transactions_category_id_fkey ( name, name_zh, icon, color )'
          )
          .eq('user_id', user.id)
          .is('deleted_at', null)
          .eq('is_hidden_from_reports', false)
