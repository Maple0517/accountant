...
| **AI 解析与智能分类** | Google `@google/genai` SDK，使用 `GEMINI_MODEL` (默认 `gemini-3.1-flash-lite`)。支持速率限制与队列批处理 |
...
/
├── src/
│   ├── middleware.ts              # Auth 守卫（Next.js 中间件）
│   ├── app/
...
│   │   │   └── settings/          # /settings Notion 配置 + iOS API Key
│   │   ├── auth/login/            # /auth/login 登录页
│   │   └── api/
│   │       ├── budget/
│   │       │   ├── category-budget/route.ts
│   │       │   └── monthly-summary/route.ts
│   │       ├── categories/route.ts
│   │       ├── cron/plaid-sync/route.ts
│   │       ├── notion/sync/route.ts
│   │       ├── plaid/
│   │       │   ├── ai-classification-jobs/process/route.ts
│   │       │   ├── create-link-token/route.ts
│   │       │   ├── exchange-token/route.ts
│   │       │   ├── sync-transactions/route.ts
│   │       │   └── webhook/route.ts
│   │       ├── receipt/route.ts       # iOS Shortcut 端点
...
│   ├── modules/
│   │   └── budget/                # ⚠️ Budget 专用模块（Clean Architecture）
│   │       ├── budget.adapter.ts
│   │       ├── budget.engine.ts
...
│   │   ├── plaid/
│   │   │   ├── ai-classification-queue.ts
│   │   │   ├── classification.ts
│   │   │   ├── client.ts
│   │   │   └── transactions-sync.ts # Plaid 手动同步 + webhook 共享逻辑
│   │   └── supabase/
...
│   ├── 003_ai_classification_queue.sql
│   ├── 004_add_plaid_item_sync_metadata.sql
│   └── 005_add_excluded_budget_categories.sql
├── docs/
│   ├── ARCHITECTURE.md            # 系统架构、Schema 参考
│   ├── PLAID_STANDARD_SYNC_DESIGN.md
...
- `/api/plaid/webhook` 收到 `ITEM` 类事件（如 `ERROR` 或 `LOGIN_REPAIRED`）时，会自动更新 `plaid_items.status` 为 `login_required`，`error` 或 `active`。
...
### 6. Auth 中间件文件名
`src/middleware.ts` 是 Next.js Auth 守卫。不要尝试去寻找或修改 `proxy.ts`，那是个已废弃的名字。
...
### 9. Budget 领域的 Clean Architecture 架构
注意 `src/modules/budget/` 文件夹。预算模块（Budget）是项目中唯一使用领域驱动设计 (Clean Architecture) 的模块，拆分为 `Engine` (纯逻辑)、`Adapter` (转化)、`Repository` (数据库) 和 `Service` (编排)。未来在修改 Budget 逻辑时，请务必遵循这种架构模式，不要直接在 API 路由里写裸 SQL。
