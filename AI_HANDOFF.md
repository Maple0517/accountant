...
| 优先级 | 类型 | 说明 |
|---|---|---|
| 🔴 | Ops | 远端 Supabase 需执行 `supabase/migrations/002_ios_receipt_api_keys.sql`（含 `receipts` 和 `api_keys` 表） |
| 🔴 | Bug | `src/middleware.ts` 有弃用警告，需改名为 `src/proxy.ts` |
| 🔴 | Bug | Dashboard 部分统计卡片仍显示 placeholder 数据 |
| 🔴 | Bug | Sidebar CSS 对齐问题（某些宽度下） |
...
/
├── src/
│   ├── middleware.ts              # Auth 守卫（⚠️ 有弃用警告，需改名为 proxy.ts）
│   ├── app/
...
│   └── 002_ios_receipt_api_keys.sql # receipts + api_keys 表（⚠️ 远端需执行）
├── docs/
...
`src/middleware.ts` 在当前 Next.js 版本需改名为 `src/proxy.ts`。功能正常，但警告持续出现。

### 7. iOS Capture API 的数据库依赖
`src/app/api/receipt/route.ts` 依赖 `supabase/migrations/002_ios_receipt_api_keys.sql`（含 `receipts` 和 `api_keys` 表）。若远端未执行此迁移，iOS Shortcut 会报数据库错误。

该 API 不依赖 Plaid：识别到图片后，自动创建/复用 `accounts.name = 'iOS Capture'` 的手动账户，写入 `source = 'receipt'` 的交易记录。
